...
PKGR="apt-get"
PACKAGES="build-essential debhelper dpkg-dev devscripts lintian fakeroot quilt equivs"
PRE_PACKAGES="apt-transport-https ca-certificates curl gnupg bc software-properties-common"
...
# If Ubuntu
if [[ "${ID}" == "ubuntu" ]]; then
    retry 5 ${PKGR} -y install software-properties-common
...
retry 5 ${PKGR} update -y
retry 5 ${PKGR} -y install ${PRE_PACKAGES} || true

# Install the core development and packaging tools
retry 5 ${PKGR} -y install ${PACKAGES} || true
